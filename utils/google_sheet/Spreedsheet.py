...
class Spreadsheet:
    ...
    def create(self, title, sheetTitle, rows=1000, cols=26, locale='ru_RU', timeZone='Etc/GMT'):
        ...
        return {'spreadsheetId': self.spreadsheetId, 'sheetId': self.sheetId, 'sheetTitle': self.sheetTitle}
    ...
    def setSpreadsheetBy(self, spreadsheetId, sheetId, sheetTitle):
        spreadsheet = self.service.spreadsheets().get(spreadsheetId=spreadsheetId).execute()
        if self.debugMode:
            pprint(spreadsheet)
        self.spreadsheetId = spreadsheet['spreadsheetId']
    ...
    def addSheet(self, sheetTitle, rows=1000, cols=26):
        ...
        return {'sheetId': self.sheetId, 'sheetTitle': self.sheetTitle}
    ...
    def clear_table(self):
        if self.sheetId is None:
            raise SheetNotSetError()
        self.requests.append({"updateCells": {"range": {"sheetId": self.sheetId}, "fields": "*"}})
    ...
    def prepare_unmergeCells(self, cellsRange):
        self.requests.append({"unmergeCells": {"range": self.toGridRange(cellsRange)}})
