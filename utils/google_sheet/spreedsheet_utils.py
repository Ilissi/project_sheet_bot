...
from utils.db_api.users_controller import get_user_id
...
def add_sum(ss, name, amount, value, index_first, first_symbol='A', second_symbol='D'):
    set_field = '{}{}:{}{}'.format(chr(ord(first_symbol) + value), index_first + 2,
                                   chr(ord(second_symbol) + value), index_first + 2, )
...
async def draw_page(ss, project_id, fields, month_list):
    ...
    for month in month_list:
        counter_value_row = 3
        for field in fields:
            add_department(ss, field['name'], counter_value_column, counter_value_row)
            ...
            for user in set(users):
                ...
                if len(list_with_pays) == 0:
                    add_user_without_pays(ss, user['user_id'], counter_value_column, counter_value_row)
                else:
                    add_pays = []
                    for pay in list_with_pays:
                        add_pays.append([values for values in pay.values()])
                    add_user_with_pay(ss, user['user_id'], add_pays, counter_value_column, counter_value_row)
                time.sleep(4)
            department_sum = await get_department_sum(field, month)
            add_sum(ss, field['name'], department_sum, counter_value_column, counter_value_row)
            counter_value_row += 1
            counter_value_row += 4
        project_sum = await get_project_sum(project_id, month)
        project_name = await get_project_name(project_id)
        add_sum(ss, project_name[0]['name'], project_sum, counter_value_column, counter_value_row)
